...
export default function DashboardHeader({
  ...
  return (
    <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
      <form
        className="min-w-[280px] max-w-[820px] flex-1"
        onSubmit={event => {
      ...
      >
        <div className="relative">
          <span
            className={`pointer-events-none absolute inset-y-0 left-4 flex items-center ${searchVariant === 'events' ? 'text-gray-500' : 'text-gray-400'}`}
          >
            <SearchIcon />
          </span>
          <input
            ...
            className={searchVariant === 'events'
              ? 'w-full rounded-xl border border-gray-300 bg-[#f6f8fc] py-3 pl-11 pr-4 text-sm text-gray-800 outline-none placeholder:text-gray-500 focus:border-violet-500'
              : 'w-full rounded-xl border border-gray-800 bg-gray-900/70 py-3 pl-11 pr-4 text-sm text-gray-100 outline-none placeholder:text-gray-500 focus:border-orange-500/60'}
          />
        </div>
      </form>
